...
std::vector<std::pair<std::string, std::string> > NewRemoteSyncManager::get_rsync_info(int instance) {
    ...
    for (iter = m->begin(); iter != m->end(); iter++) {
        ...
        v.push_back(std::pair<std::string, std::string>(iter->second->sid(), info));
    }
    return v;
}
    ...
    if (is_rsync_remote_config_equals(remotes) == true) {
        ...
        for (size_t i = 0; i < remotes.size(); i++) {
            ...
            if (iter == m->end()) {
                ...
                UNUSED(ref);
                assert(ref == 1);
                m->insert(std::pair<std::string, RemoteClusterHandler *>(handler->get_info(), handler));
            } else {
                int ref = iter->second->Ref(); // incr ref
                UNUSED(ref);
                assert(ref == 2);
                m->insert(std::pair<std::string, RemoteClusterHandler *>(iter->first, iter->second));
