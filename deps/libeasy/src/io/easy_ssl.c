 ...
 * 初始化ssl
 */
__attribute__((unused)) static unsigned long id_function(void)
{
    return ((unsigned long) pthread_self());
}

__attribute__((unused)) static void locking_function(int mode, int type, const char *file, int line)
{
    if (mode & CRYPTO_LOCK) {
...
static int easy_ssl_handshake(easy_connection_t *c)
{
    ...
    if (n == 1) {
       ...
       // }
        ...
        if (c->sc->connection->s3 && SSL_is_server(c->sc->connection)) {
            c->sc->connection->s3->flags |= SSL3_FLAGS_NO_RENEGOTIATE_CIPHERS;
        }
...
static int easy_ssl_generate_rsa512_key(easy_ssl_ctx_t *ssl)
{
    ...
    key = RSA_generate_key(512, RSA_F4, NULL, NULL);
      ...
      RSA_generate_key_ex(key, 512, bne, NULL);
